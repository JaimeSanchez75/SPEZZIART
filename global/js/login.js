document.addEventListener('DOMContentLoaded', () =>  // Este script se encarga de manejar la lógica de los formularios de login y registro, así como el cambio de tema según las preferencias del sistema. También incluye funciones para mostrar alertas dinámicas en la página.
{
    ...
    if (loginForm) handleFormSubmit(loginForm, '/auth/login', '/');
    if (registerForm) handleFormSubmit(registerForm, '/auth/register', null); //No redirige automáticamente tras el registro, solo muestra mensaje y cambia al login.
});
...
class individualController 
{
    ...
    public function crear() 
    {
        ...
        if (!empty($_GET['id'])) {
            ...
            if (!$receta) {
                die("Receta no encontrada o no tienes permiso para editarla.");
            }
        }
    ...
    public function guardar() 
    {
        ...
        try 
        {
            ...
            if (!empty($_POST['id'])) {
                ...
                $id = (int)$_POST['id'];
                if (!$model->actualizarReceta($id, $userId, $data)) {
    ...
    public function ver() 
    {
        ...
        if (!$receta) {
            die("Receta no existe o no tienes permiso para verla.");
        }
    ...
    public function verColeccion() 
    {
        ...
        if (!$coleccion) {
            die("Colección no encontrada o no tienes acceso.");
        }

        $recetas = $model->getRecetasDeColeccion($coleccionId);
        $config = [
            'ModoOscuro' => Auth::user()['ModoOscuro'] ?? false
        ];
    ...
    public function agregarReceta() 
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST' 
            ...
            if (!$model->getColeccionByIdAndUser($idColeccion, $userId)) {
                die("No tienes permiso para modificar esta colección.");
            }

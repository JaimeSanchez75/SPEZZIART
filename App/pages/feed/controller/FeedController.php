...
class FeedController {
    public function index() {
        ...
        (new FeedView())->render($posts, $etiquetas, $catFiltro);
    }
    ...
    public function filtrar() {
        ...
        $busqueda = $_POST['busqueda'] ?? null; // Importante: recoger el texto
    ...
    public function crearPost() {
        ...
        $model = new FeedModel();
        if ($model->crearPost(Auth::id(), $titulo, $descripcion)) {
            header("Location: /App/pages/feed?success=1");
        } else {
            header("Location: /App/pages/feed?error=db");
        }
        exit;
    }
}